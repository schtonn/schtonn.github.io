...
function binl2b64(d) {
    for (var r = '', _ = 0; _ < 4 * d.length; _ += 3)
        for (var m = (d[_ >> 2] >> _ % 4 * 8 & 255) << 16 | (d[_ + 1 >> 2] >> (_ + 1) % 4 * 8 & 255) << 8 | d[_ + 2 >> 2] >> (_ + 2) % 4 * 8 & 255, n = 0; n < 4; n++)
            8 * _ + 6 * n > 32 * d.length ? r += b64pad : r += 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'.charAt(m >> 6 * (3 - n) & 63);
    return r;
}